...
def generate_sum_feature(pre_hiddens, num_sum=128):
    """
        args: 
            pre_hiddens: [tensor] a 'time_step' length list of tensor with shape: [batch_szie, num_hiden]
            num_sum: [int] number of summarization layer
        return:
            sum_feature: [tensor] a 2D 
    """
    
    time_steps = len(pre_hiddens)
    batch_size, num_pre_hiddens = pre_hiddens[0].get_shape().as_list()

    weights = tf.Variable(tf.random_normal([num_pre_hiddens, num_sum]), dtype=tf.float32)
    biases = tf.Variable(tf.random_normal([num_sum]), dtype=tf.float32)
    sum_feature = tf.constant(0.0, shape=[batch_size, num_sum])
    for i in range(time_steps):
        sum_feature += tf.nn.tanh(tf.matmul(pre_hiddens[i], weights) + biases)
...
def danamic_layer_norm(inp, pre_hiddens, eps=1e-5, scope=None):
    """
        args: 
            inp: [tensor] a 2D tensor with shape: [batch_size, num_hidden]
            pre_hiddens: [tensor] a 'time_step' length list of tensor with shape: [batch_szie, num_hiden]
    ...
    # time_steps = len(pre_hiddens)
    batch_size, num_pre_hiddens = pre_hiddens.get_shape().as_list()
  
    scope = '' if scope == None else scope
    with tf.variable_scope(scope+'layer_norm'):
        sum_feature = generate_sum_feature(pre_hiddens) #tensor shape: [batch_size, num_sum]
        num_sum = sum_feature.get_shape().as_list()[1]
        gain_weights = tf.Variable(tf.random_normal([num_sum, num_hidden]), dtype=tf.float32)
        gain_biases = tf.Variable(tf.random_normal([num_hidden]), dtype=tf.float32)

        gain = tf.matmul(inp, gain_weights) + gain_biases #tensor shape: [batch_size, num_hidden]
        bias = tf.get_variable('bias', shape=[inp.get_shape()[1]], initializer=tf.constant_initializer(0))
...
class DanamicLayerNormLSTMCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            i = danamic_layer_norm(i, inputs, scope='i/')
            j = danamic_layer_norm(j, inputs, scope='j/')
            f = danamic_layer_norm(f, inputs, scope='f/')
            o = danamic_layer_norm(o, inputs, scope='o/')
            ...
            new_h = self._activation(danamic_layer_norm(new_c, inputs, scope='h/')) * tf.nn.sigmoid(o)
