...
class HyperLSTMCell(tf.contrib.rnn.RNNCell):
  ...
  def __call__(self, x, state, timestep = 0, scope=None):
    with tf.variable_scope(scope or type(self).__name__):
      ...
      # shape: [batch_size, x_size+num_units]
      hyper_input = tf.concat([x, h], 1) 
      # hyper_output shape: [batch_size, x_size+num_units]
      hyper_output, hyper_new_state = self.hyper_cell(hyper_input, hyper_state)

      xh = tf.matmul(x, W_xh) #shape: [batch_size, 4*num_units]
      hh = tf.matmul(h, W_hh)

      # split Wxh contributions
      ix, jx, fx, ox = tf.split(xh, 4, 1) # each tensor has shape: [batch_size, num_units]
